...
public class DynamicBoneDemo1 : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        UnityEngine.GUI.Label(new Rect(x, y, w2, h), "Press arrow key to move");
        Animation a = m_Player.GetComponentInChildren<Animation>();
        y += h;
        a.enabled = UnityEngine.GUI.Toggle(new Rect(x, y, w2, h), a.enabled, "Play Animation");

        y += h * 2;
        DynamicBone[] dbs = m_Player.GetComponents<DynamicBone>();
        UnityEngine.GUI.Label(new Rect(x, y, w2, h), "Choose dynamic bone:");
        y += h;
        dbs[0].enabled = dbs[1].enabled = UnityEngine.GUI.Toggle(new Rect(x, y, w1, h), dbs[0].enabled, "Breasts");
        y += h;
        dbs[2].enabled = UnityEngine.GUI.Toggle(new Rect(x, y, w1, h), dbs[2].enabled, "Tail");

        y += h;
        UnityEngine.GUI.Label(new Rect(x, y, w2, h), "Weight");
        m_weight = UnityEngine.GUI.HorizontalSlider(new Rect(x + 50, y, w1, h), m_weight, 0, 1);
